#!/bin/bash
BURIQ () {
    curl -sS https://raw.githubusercontent.com/zalsfyn/akses/main/ipvps > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/zalsfyn/akses/main/ipvps | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/zalsfyn/akses/main/ipvps | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
Exp=$(curl -sS https://raw.githubusercontent.com/zalsfyn/akses/main/ipvps | grep $MYIP | awk '{print $3}')
fi
...
9) clear ; genssl ;;
10) clear ; jam ;;
6969) clear ; wget https://raw.githubusercontent.com/zalsfyn/v4/main/update.sh && chmod +x update.sh && ./update.sh && rm -f /root/update.sh ;;
0) clear ; menu ;;
x) exit ;;
